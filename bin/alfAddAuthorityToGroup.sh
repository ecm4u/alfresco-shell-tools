...
function __show_command_explanation() {
  ...
  echo "    the alfAddAuthorityToGroup.sh adds a user or a group to an existing group."
  echo
  echo "  usage examples:"
  echo
  echo "  alfAddAuthoritytoGroup.sh GroupA user123"
  echo "     --> Adds the user user123 as a member of the group GroupA"
  echo
  echo "  alfAddAuthoritytoGroup.sh ParentGroup GROUP_subgroup"
  echo "     --> adds the group subgroup as a sub group of the group ParentGroup" 
  echo
}
...
curl $ALF_CURL_OPTS -u $ALF_UID:$ALF_PW -H 'Content-Type:application/json' -X POST "$ALF_EP/service/api/groups/$ENC_GROUP/children/$ENC_AUTHORITY"
